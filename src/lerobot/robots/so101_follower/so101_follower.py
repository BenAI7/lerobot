...
class SO101Follower(Robot):
    ...
    def _cameras_ft(self) -> dict[str, tuple]:
        ...
        for cam_key in self.cameras:
            ...
            # Default: one RGB image per configured camera key.
            if not isinstance(cfg, OpenCVCameraConfig) or cfg.split_mode is None:
                features[cam_key] = (cfg.height, cfg.width, 3)
                continue

            # Packed stereo: expose left/right as separate camera keys in the observation/dataset.
    ...
    def get_observation(self) -> dict[str, Any]:
        ...
        for cam_key, cam in self.cameras.items():
            ...
            # If configured, split packed stereo into left/right keys.
